...
double getPi(pthread_t* threadsID, struct ThreadInfo* threadsInfo, int threadsAmount){
	int i, threadJoinReturn = PTHREAD_JOIN_SUCCESS;
	double pi = 0;
	for(i = 0; i < threadsAmount && threadJoinReturn == PTHREAD_JOIN_SUCCESS; i++){
		threadJoinReturn = pthread_join(threadsID[i], NULL); 
		pi+= (threadsInfo[i]).threadPi;
	}
...
int main(int argc, char **argv){
	...
	double pi = getPi(threadIDs, threadsInfo, threadsAmount);
	if (pi == EXIT_FAILURE){
		freeMemory(threadIDs, threadsInfo);
		return EXIT_FAILURE;
	}
